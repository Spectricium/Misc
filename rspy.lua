...
local function ErrorPrompt(Message,state)
    if getrenv then
        ...
        local prompt = ErrorPrompt.new("Default",{HideErrorCode = false})
...
local saveinstance = saveinstance or function()
    ErrorPrompt("Save Instance Is Missing")
end
local decompile = decompile or function()
    ErrorPrompt("decompile Is Missing")
end
...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "Simple Spy Synapse Edition",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
function genScript(remote, args)
    ...
    prevTables = {}
    return "--Generated By Simple Spy Synapse Edition\n"..gen
end
...
if not getgenv().SimpleSpyExecuted then
    local succeeded,err = pcall(function()
        ...
        log(spawn(function()
            codebox:setRaw("Welcome "..cloneref(game:GetService("Players").LocalPlayer).DisplayName)
        end))
...
function()
    ...
    if func then
        ...
        codebox:setRaw("-- Generated by the SimpleSpy V3 synapse edition serializer\n\n"..selected.Function)
        TextLabel.Text = "Done! Function info generated by the SimpleSpy V3 synapse edition Serializer."
    else
...
newButton(
    "InfYield DEX",
    function() return "Load's Infinite Yield DEX" end,
    function()
...
local bugiwugy = ""
newButton("Get Loaded Modules",function()
    return "Get's All Loaded Modules"
end,
function()
    bugiwugy = ""
    for i, v in pairs(getloadedmodules()) do
        print("["..tostring(i).."] "..v.Name.." Path: ".."game."..v:GetFullName())
        bugiwugy = bugiwugy.."["..tostring(i).."] "..v.Name.." Path: ".."game."..v:GetFullName().."\n"
    end
    writefile("modules game "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name,bugiwugy)
end)
newButton("Get scripts",function()
    return "Get's All scripts"
end,
function()
    bugiwugy = ""
    ...
    writefile("scripts game "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name,bugiwugy)
end)
...
newButton("Load DEX",function()
    return "Load's Up Synapse DEX"
end,
function()
...
if cloneref(game:GetService("CoreGui")):FindFirstChild("SynapseDex") ~= nil then
    cloneref(game:GetService("CoreGui")):FindFirstChild("SynapseDex"):Destroy()
end
local namer = "SynapseDex"
Dex.Name = namer
if gethui then
    Dex.Parent = gethui();
